...
export async function prepareAndSendArchive(qrCodes: ProcessedQRData<AllRequests>[], response: Response) {
    try {
...
function appendQRCodesToArchive({response, qrCodes, archive}: {
    archive: Archiver,
    ...
    for (const [index, qrCode] of qrCodes.entries()) {
        ...
        try {
            ...
            throw new Error(`QR code at index ${index} is not in the expected format.`);
        }
