...
export const TabRenderer: React.FC<TabParameters> = ({ tab }) => {
  const { input, sectionTitle, label, section, fieldContainer } = styles;

  const { dispatch, state, setError, selectedCrypto } = useCore();

  const isFieldRequired = (tab: Tabs, fieldName: string): boolean => {
    const tabMapping = requiredFieldsMapping[tab];
