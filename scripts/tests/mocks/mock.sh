...
function setup_and_run_mock() {
  ...
  if [[ ${config} == "letsencrypt" ]]; then
    apply_profile "${INSTALL_PROFILE}" "production_install_config"
    setup_letsencrypt_mock_parameters
  elif [[ ${config} == "self-signed" ]]; then
    apply_profile "${INSTALL_PROFILE}" "ssl_dev_config"
    setup_self_signed_mock_parameters
  elif [[ ${config} == "dev" ]]; then
    apply_profile "${INSTALL_PROFILE}" "http_dev_config"
  fi
