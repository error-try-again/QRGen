...
function check_and_generate_certificates() {
  ...
  if [[ ! -f "${fullchain_path}" ]] || [[ ${REGENERATE_SSL_CERTS} == "true" ]]; then
    openssl req -x509 -nodes -days 365 -newkey rsa:4096 \
      -keyout "${privkey_path}" \
      -out "${fullchain_path}" \
      -subj "/CN=${DOMAIN_NAME}"
    print_messages "Private key generated at ${privkey_path}." "Certificate generated at ${fullchain_path}."
  else
...
function handle_dh_param_generation() {
  ...
  if [[ ! -f "${DH_PARAMS_FILE}" ]]; then
    ...
    generate_dh_parameters
  elif [[ "${REGENERATE_SSL_CERTS}" == "true" ]]; then
    generate_dh_parameters
  elif [[ "${REGENERATE_SSL_CERTS}" == "false"  ]]; then
    prompt_for_dhparam_regeneration && generate_dh_parameters
  else
...
function prompt_for_dhparam_regeneration() {
  read -rp "Do you want to regenerate the certificates? [y/N]: " response
