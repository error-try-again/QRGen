...
handle_certs() {
  ...
  if [[ $USE_LETS_ENCRYPT == "yes" ]] || [[ $USE_SELF_SIGNED_CERTS == "yes" ]]; then
    ...
    # initialize_cert_watcher() is currently disabled due to side effects caused by
    # automagically restarting
    # the frontend and backend services in the background due to manual updates to
    # the certs. This causes the services to be restarted multiple times in a
    # non-optimal run.
